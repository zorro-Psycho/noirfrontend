...
const Contact = () => {
  return (
    <>
      <Header />
      <div
        className="relative bg-scroll text-center bg-cover bg-left bg-no-repeat py-10"
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-xl sm:text-2xl text-center bg-gradient-to-r from-gray-900 via-gray-700 to-gray-900 p-6 sm:p-10 text-white mb-12 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 lg:gap-12">
            {teamMembers.map((member, index) => (
              <div
                key={index}
                className="bg-black rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300"
              >
                <img
                  src={member.image}
                  alt={member.name}
