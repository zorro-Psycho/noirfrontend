  ...
  const handleDeleteGameSession = async (sessionId) => {
    ...
    try {
      ...
      setProfileData((prevData) => ({
        ...prevData,
        gameSessions: prevData.gameSessions.filter((session) => session.id !== sessionId)
      }));
    } catch (err) {
      setError(err.message);
    }
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            ...
            <div className="bg-gray-700 p-4 rounded-lg mb-6">
              ...
              {profileData.achievements.length > 0 ? (
                <ul>
                  {profileData.achievements.slice(0, visibleAchievements).map((achievement) => (
              ...
              {profileData.achievements.length > visibleAchievements && (
                <button onClick={handleShowMoreAchievements} className="text-blue-500">See More</button>
              )}
              {visibleAchievements > 5 && (
                <button onClick={handleShowLessAchievements} className="text-blue-500">See Less</button>
              )}
            </div>
            <div className="bg-gray-700 p-4 rounded-lg mb-6">
              ...
              {profileData.gameSessions.length > 0 ? (
                <ul>
                  {profileData.gameSessions.slice(0, visibleGameSessions).map((session) => (
                    <li key={session.start_time} className="mb-4 flex justify-between items-center">
                      <div>
                        <p>Game: {session.title || 'N/A'}</p>
                        <p>End Time: {new Date(session.start_time).toLocaleString() || 'N/A'}</p>
              ...
              {profileData.gameSessions.length > visibleGameSessions && (
                <button onClick={handleShowMoreGameSessions} className="text-blue-500">See More</button>
              )}
              {visibleGameSessions > 5 && (
                <button onClick={handleShowLessGameSessions} className="text-blue-500">See Less</button>
              )}
            </div>
          </div>
        </div>
